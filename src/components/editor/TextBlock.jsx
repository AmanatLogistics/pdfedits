...
export default function TextBlock({
  ...
  // Declared after startEdit/doCommit so the effects close over the current
  // callbacks rather than bindings that are still in their temporal dead zone.
  // forceEdit from parent (context toolbar "Edit" button)
  useEffect(() => {
    if (forceEdit && !editing) startEdit()
    else if (!forceEdit && editing) doCommit()
    // Intentionally keyed on forceEdit alone: this mirrors the parent's
    // request, and depending on `editing` would immediately undo it.
