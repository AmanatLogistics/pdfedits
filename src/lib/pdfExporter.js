...
async function exportVectorPdf(originalArrayBuffer, editLayers, pageCount, pageBgs, blockBgs) {
  // Work on a copy: PDF.js transfers (and detaches) any buffer handed to it,
  // and the same source buffer is reused across renders and exports.
 ...
 *
 * The vector path is the default because it keeps every page as real,
 * selectable, searchable text. The visual path re-renders each edited page to
 * a bitmap: it reproduces the on-screen preview exactly, but the page stops
 * being text — nothing can be selected, searched, copied or re-edited, it
 * blurs when zoomed, and the file grows by an order of magnitude. That is only
 * an acceptable trade when the caller explicitly asks to flatten, or when the
 * vector export cannot run at all.
 *
 * @param {ArrayBuffer} originalArrayBuffer  the source PDF
 * @param {object} editLayers                per-page { texts, annotations }
 * @param {number} pageCount
 * @param {object} pageBgs                   page-level sampled background colours
 * @param {object} blockBgs                  per-block sampled background colours
 * @param {{ flatten?: boolean }} [options]  flatten: rasterise edited pages
 */
export async function exportPdf(
  ...
  try {
    ...
    // Falling back keeps the download working, but the result is a picture of
    // the document rather than the document, so say so loudly.
    console.warn(
      'Text-preserving export failed; falling back to a flattened (image) export. ' +
        'The result will not contain selectable text.',
      err,
    )
    return await exportVisualPdf(originalArrayBuffer, editLayers, pageCount, pageBgs)
  }
...
export function downloadBytes(bytes, filename) {
  ...
  a.click()
  a.remove()
  // A large PDF can take a moment to start saving; don't pull the URL early.
